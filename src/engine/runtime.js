...
class Runtime extends EventEmitter {
    ...
    _convertBlockForScratchBlocks(blockInfo, categoryInfo) {
        ...
        const blockJSON = {
            type: extendedOpcode,
            inputsInline: true,
            category: blockInfo.category || categoryInfo.name,
            colour: categoryInfo.color1,
            colourSecondary: categoryInfo.color2,
            colourTertiary: categoryInfo.color3
        };
    ...
    getBlocksXML(target) {
        return this._blockInfo.map(categoryInfo => {
            ...
            const defaultCategoryName = categoryInfo.name;

            paletteBlocks.forEach(block => {
                const catName = block.info.category || defaultCategoryName;
                if (!blocksByCategory[catName]) blocksByCategory[catName] = [];
                blocksByCategory[catName].push(block);
            });

            const categories = Object.keys(blocksByCategory);
            let innerXML = '';

            // If we have multiple categories, or the single category has a custom name different from the extension
            if (categories.length > 1 || (categories.length === 1 && categories[0] !== defaultCategoryName)) {
                categories.forEach((catName, index) => {
                    const groupBlocks = blocksByCategory[catName];
                    const groupXML = groupBlocks.map(block => block.xml).join('');

                    // Add separator between groups (but not before the first one)
                    if (index > 0) {
                        innerXML += '<sep gap="36"/>';
                    }

                    // Add label for the category
                    innerXML += `<label text="${xmlEscape(catName)}"></label>`;

                    // Add blocks
                    innerXML += groupXML;
                });
            } else {
                // Fallback to original behavior: flat list
                innerXML = paletteBlocks.map(block => block.xml).join('');
            }

            return {
                id: categoryInfo.id,
                xml: `<category name="${name}" id="${categoryInfo.id
                    }" ${statusButtonXML} ${colorXML} ${menuIconXML}>${innerXML}</category>`
            };
        });
    }
